...
function shareProduct(platform) {
  ...
  const pageTitle = encodeURIComponent(`Check out ${productName} - ${productPrice} at POStore!`);
  ...
  switch(platform) {
    ...
    case 'pinterest':
      ...
      break;
    case 'email':
      shareUrl = `mailto:?subject=${pageTitle}&body=I thought you might be interested in this: ${pageUrl}`;
      break;
  }
...
function shareProductDirect(platform, product) {
  // Create sharing URL
  const pageUrl = encodeURIComponent(window.location.href);
  const pageTitle = encodeURIComponent(`Check out ${product.name} - $${product.price} at POStore!`);
